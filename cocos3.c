...
{
  /* variables locals */ 
  int id_fantasma,n, mida_camp;  
  char id_fant[20], str_files[30], str_columnes[30],str_fi1[20], str_fi2[20], ret_joc[30];
  void *p_fantasma;
	...
	for (int i=0; i<num_fantasmes; i++){
		...
		if (tpid[n] == (pid_t) 0){ 
			sprintf(fil_fant, "%i", f1[i].f); 
		   	...
		   	sprintf(id, "%i",i);
		   	execlp("./fantasma3","fantasma3", id_fant, str_files, str_columnes,fil_fant, col_fant, dir_fant, ret_fant, act_fant,id,ret_joc,str_fi1, str_fi2, (char*)0); 
		   	exit(0); 
		}
		else if (tpid[n] > 0) n++; 
		}
